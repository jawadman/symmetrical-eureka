...
function preload() {
  ...
  for (let i = 1; i <= totalIdleFrames; i++) {
    idleFrames.push(loadImage(`idle_${i}.png`));
  }

  for (let i = 1; i <= totalRollFrames; i++) {
    rollFrames.push(loadImage(`roll_${i}.png`));
  } 
  for (let i = 1; i <= totalAttackFrames; i++) {
    attackFrames.push(loadImage(`2_atk_${i}.png`));
  } 
  for (let i = 1; i <= totalBlockFrames; i++) {
    blockFrames.push(loadImage(`defend_${i}.png`));
  } 
  for (let i = 1; i <= totalRunFrames; i++) {
    runFrames.push(loadImage(`run_${i}.png`));
  } 
}

function setup() {
  bg = loadImage('Desert_bg.jpg');
